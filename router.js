...
export function makeInboundHandler(ctx, cfg, getCompanyId) {
  return async function onInbound(msg, tx) {
    ...
    if (!prompt) {
      await tx.sendText(target, `Hi, this is @${alias}. What would you like?`);
      return;
    }

    // Hand the prompt to the agent as an issue; the reply comes back on completion.
    let issue;
    try {
      issue = await ctx.issues.create({
        companyId,
        title: `[TG] ${truncate(prompt, 180)}`,
        description: prompt, // always write the full message to the ticket description
    ...
    // First-person ack: the bot you messaged is the single point of contact and reports
    // back here (results or needs), regardless of any internal delegation to other agents.
    await tx.sendText(target, "🤖 On it — working on this now; I'll report back here.", { silent: true });
  };
}
